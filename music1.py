...
class Song:
    ...
    def create_embed(self):
        #song = api.search_song("{0.source.title}".format(self))
        embed = (discord.Embed(title='Now playing',
                 ...
                 #.add_field(name='Lyrics', value=song.lyrics)
...
api= genius.Genius('jZKmtdZmoviQYLseRQDOKJDwPv6pLNlximw5v1dkHsTDrz3OZKp3pYXgtZPJMTOJ')
...
class Music(commands.Cog):
    ...
    @check_toggle()
    async def _lyrics(self, ctx: commands.Context, *, song_details: str = None):
        ...
        artist_name, song_name = song_details.split(", ")
        try:
            artist = api.search_artist(f"{artist_name}", max_songs=1, sort="title")
            song = api.search_song(f"{song_name}", artist.name)
            paras = []
            s = song.lyrics
            paras = s.split("\n\n")

            for i in range(len(paras)):
                await ctx.channel.send(f"{paras[i]}\n------")
        except:
            await ctx.channel.send("There is some issue... :(")
